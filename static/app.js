...
# ✅ Hide sklearn "valid feature names" warnings (and we also prevent them properly below)
...
# -------------------------
DEFAULT_GST_RATE = 0.03           # approx
DEFAULT_RETAIL_MARGIN_KG = 25000  # adjust as you want
...
def ensure_dataframe_with_feature_names(model, X):
    """
    ✅ Always return a DataFrame with correct feature names.
    This prevents: "X does not have valid feature names..."
    """
    # If already DataFrame, keep it
    if isinstance(X, pd.DataFrame):
        X_df = X.copy()
    else:
        # If it's numpy/series/list -> convert to DataFrame
        X_df = pd.DataFrame(X)

    # If model knows feature names, enforce order + names
    if hasattr(model, "feature_names_in_"):
        cols = list(model.feature_names_in_)

        # If X_df has no meaningful column names or wrong count, force columns
        if X_df.shape[1] == len(cols):
            X_df.columns = cols

        # Reindex to exact expected cols (fills missing with 0)
...
def predict_with_model(model, X_row):
    ...
    """
    X_ready = ensure_dataframe_with_feature_names(model, X_row)
...
def fetch_hourly_series(days="5d"):
    try:
        ...
        return safe_close_series(silver)
    except Exception:
        return None
...
# ✅ Railway gunicorn entry: gunicorn app:app
